...
void Showinfo(const guestinfo info,const roomtype room,int loc){ //ฟังก์ชันของอิ๋ว
    cout<<"\n\n\n\n-------------------------------------------------------------\n";
...
void booking(roomtype &room,guestinfo &info,vector<guestinfo> &roomstatus){ //ฟังก์ชันของวอ
...
int main() {
    ...
    for(int i=1;i<10;i++){
        if(checkuser()==1){ //ถ้าcheckuser == 1 จะเข้าสู่เมนูของลูกค้า
            if(guestpart()==1){
                searchforGuest(info,room);//เพิ่มฟังก์ชันหาของอัพ
            }else{
            ...
            for(int i=0;i<4;i++){ //ลูปให้ใส่ username,password ได้ไม่เกิน 3 ครั้ง
                if(receplogin()==1){
                    cout << "Welcome!";
                    searchforRecep(info,room);//เพิ่มฟังก์ชันหาของอัพ
                    break;
                }
                else{
                    if(i==3){
